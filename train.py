...
# from model.loss.CDLoss import CDWithLogitsLoss
def train_net(f,f1,f_epoch_loss,iter,net, device,data_path, epochs, batch_size, ModelName, is_Transfer):
    ...
    for epoch in range(1, epochs+1):
        ...
        for image1, image2, label in train_loader:
            ...
            label=label.long()
            x_onehot = torch.zeros(label.shape[0], 2, args.patch,args.patch).long()
...
if __name__ == '__main__':
    ...
    device="cpu"
    iter=1
    f=open('txt/train_'+str(iter)+'.txt','w')
    ...
    data_path="./data/LEVER-138"
    train_net(f,iter,net, device, data_path,50,3,0.0001,'siam_diff',is_Transfer= False)