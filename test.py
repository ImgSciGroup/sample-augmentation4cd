...
def test(net,iter,f_acc,f_time,ModelName) :
    ...
    # 读取所有图片路径
    tests1_path = glob.glob('./data/LEVER-138/pred/image1/*.tif')
    tests2_path = glob.glob('./data/LEVER-138/pred/image2/*.tif')
    label_path = glob.glob('./data/LEVER-138/pred/label/*.bmp')
    # 遍历所有图片
    num = 1
    TPSum = 0
    TNSum = 0
    ...
    for tests1_path, tests2_path, label_path in zip(tests1_path, tests2_path, label_path):
        ...
        cv2.imwrite("./data/LEVER-138/pred/CMI/"+str(iter)+"_CMI.png",pred)
